...
Public Module CodingControl
	...
	Public Function RandomPassword(ByVal MaxPasswordChars As Integer) As String
        ...
        PasswordString = "1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz"


        Return RandomPassword(MaxPasswordChars, PasswordString)
    End Function

    Public Function RandomPassword(ByVal MaxPasswordChars As Integer, PasswordString As String) As String
        Dim I As Integer
        ...
        For I = 1 To MaxPasswordChars
            CharIndex = RandomCreator(0, PasswordString.Length - 1)
            'CharIndex = CInt(Int(PasswordString.Length * Rnd()))
    ...
    Public Function GetMD5WithoutBase64(ByVal Input As String) As String
        Dim md5Hasher As System.Security.Cryptography.MD5 = System.Security.Cryptography.MD5.Create() '建立一個MD5物件
        Dim data As Byte() = md5Hasher.ComputeHash(Encoding.UTF8.GetBytes(Input)) '將input轉換成MD5，並且以Bytes傳回，由於ComputeHash只接受Bytes型別參數，所以要先轉型別為Bytes
        Dim sBuilder As New StringBuilder() '建立一個StringBuilder物件
        Dim i As Integer                    '將Bytes轉型別為String，並且以16進位存放
        For i = 0 To data.Length - 1
            sBuilder.Append(data(i).ToString("x2"))
        Next i
        Return sBuilder.ToString() '傳回
    End Function
